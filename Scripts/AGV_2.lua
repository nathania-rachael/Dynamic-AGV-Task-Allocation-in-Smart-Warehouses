...
function sysCall_init()
    agvName = "AGV_2" -- Change this to "AGV_2" for the second AGV
    ...
    dockHandle = sim.getObjectHandle("WP_14") -- dock for AGV_1
    ...
    wp14= sim.getObjectHandle("WP_14")
    wp17= sim.getObjectHandle("WP_17")
    ...
    routes = {
     ...
     {"Drop_1", "Pickup_4", {wp5, wp1, wp9, wp10,wp11,wp12}},
     ...
     -- Docking routes for AGV_2 (Drop -> WP_14)
     -- NOTE: wp14 included here so goToDock() does NOT need to append dockHandle
     {"Drop_1", "WP_14", {wp5, wp1, wp2, wp3,wp17,wp14}},
     {"Drop_2", "WP_14", {wp6, wp2, wp3,wp17,wp14}},
     {"Drop_3", "WP_14", {wp7, wp3,wp17,wp14}},
     {"Drop_4", "WP_14", {wp8, wp17, wp14}}
}
    routeDict = {}
...
-- GO TO DOCK: builds fullRoute using routeDict (WP_14 must be in dockRoute)
function goToDock(fromDrop)
    local key = fromDrop .. "|WP_14"
    ...
    if dockRoute then
        ...
        -- do NOT append dockHandle here ? the dock (wp14) should be included in dockRoute
        currentTarget = 1
        ...
        if currentTarget > #fullRoute then
            ...
            if currentTask then
                ...
                if returningToDock then
                    print(agvName .. " reached dock (WP_14) ? stopping and parked")
                    returningToDock = false
